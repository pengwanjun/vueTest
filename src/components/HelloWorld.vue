<template>
	<div class="hello">
		<ul tabindex="1" @keyup="changeFocus($event)">
			<li :class="{'active':item.focus,'canOpera':item.canOpera}" v-for="item in $store.state.helloWorld.baseData">
				<template1 ref="t1" v-if="item.templateType==1" :value="[item.name,item.value[item.curVlaue],item.canOpera]"></template1>
				<template2 ref="t2" v-else-if="item.templateType==2" :value="item.index"></template2>
...
<script>
	...
	export default {
		...
		mounted() {
//			this.sliceArr(this.$store.state.helloWorld.baseData, this.stage);
		},
		methods: {
			//改变数据
//			change_num(argu){
...
//			},
			...
			//键盘操作
			changeFocus(e) {
				let baseData=this.$store.state.helloWorld.baseData;
				//下键
				if(e.keyCode == 40) {
					baseData[this.focusIndex].focus = false;
					if(this.focusIndex == baseData.length - 1) {
						this.focusIndex = 0;
						baseData[0].focus = true;
//						this.pageNum = 0;
					} else {
						for(let i = this.focusIndex + 1; i < baseData.length; i++) {
//							if(i % this.stage == 0) {
//								this.pageNum++;
//							}
							if(baseData[i].canOpera) {
								this.focusIndex = i;
								break;
							}
						}
						baseData[this.focusIndex].focus = true;
					}
...
//							}
				...
				if(e.keyCode == 39 || e.keyCode == 37) {
					let temType = baseData[this.focusIndex].templateType;
					if(temType == 1) {
